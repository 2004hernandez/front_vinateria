...
export default function ProductoDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchRecomendados = async () => {
      try {
        const res = await fetch(`${CONFIGURACIONES.BASEURL2}/productos/recomendados?id=${id}`);
        console.log("🚀 fetchRecomendados response:", res);
        const data = await res.json();
        console.log("🚀 fetchRecomendados data:", data);
        setRecomendados(data.productos || []);
      } catch (err) {
  ...
  return (
    <div className={`${light("bg-gray-50 text-gray-900")} ${dark("bg-gray-900 text-gray-100")} min-h-screen py-8 pt-36 px-4`}>
      ...
      {recomendados.length > 0 && (
        <div className={`-ml-28 mt-16 w-screen relative px-8 py-8 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
          <div className="container mx-auto max-w-7xl">
            <h2 className={`text-2xl font-bold mb-6 ${theme === "dark" ? "text-gray-100" : "text-black"}`}>
                ...
                {recomendados.map((prod) => (
                  <Card
                    ...
                    className={`
                      ...
                      group cursor-pointer hover:shadow-lg transition-all duration-300 hover:-translate-y-1
                      ${light("bg-white text-gray-900")} ${dark("bg-gray-800 text-gray-100")}
                    `}
                  ...
                  >
                    <CardContent className="p-0">
                      <div className="relative overflow-hidden rounded-t-lg">
                        ...
                        <Badge className={`absolute top-2 right-2 ${prod.stock > 0 ? "bg-green-500" : "bg-red-500"}`}>
                      ...
                      <div className="p-4">
                        <h3
                          className={`${light("text-black")} ${dark("text-blue-400")} font-bold text-lg mb-1 line-clamp-1`}
                        >
                          {prod.name}
                        </h3>
